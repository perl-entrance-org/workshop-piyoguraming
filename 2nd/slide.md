...
- Perlでは文字列を `"` か `'` のどちらかで囲みます
    - 文字列とは、プログラミング中でデータとして使う、2文字以上の文字の並びのことを指します
...
- `()` の中に真偽値をいれることで動きが変化します
    - このことを「条件分岐」と呼びます
- scratchの「でなければ」は `else` が相当します
...
- ifの`()` の中に数値をいれてみて、どのような動きをするか確かめてみよう
...
- 変数 `$ten` に10 を 変数 `$five` に5を代入しよう
- 上の論理演算子を使って if文を書いてみよう
    - 0以上(0を含む)かどうか
    - 100ではないか
    - 20より大きいか
    - 20より小さいか
...
```perl
    ...
    le  # less equal
```
- Perlでは,文字列を比較する場合は`eq`や`ne`といった単語を用いた比較演算子を使います
...
- このif文の条件式を変えて　「 10以上で偶数ならprintさせる」ようにしてみましょう
...
- カウンタ変数が `$i` なのは indexの略で、慣例的にループのちょっとしたカウンタ変数ではiを使います
...
- 通常は `{` `}`で囲まれている「ブロック」の中にスコープが限定されます
...
- このfor文では, 変数 `$i` のスコープはfor文のブロックの中に限定されるので、外からは使えません
...
1. - -10から10までの集合を作ってみましょう
   - それをfor文で出力してみましょう

2. - 10回「Hello!!!」と叫ぶなぞのプログラムを書いてみましょう
